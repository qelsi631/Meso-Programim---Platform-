...
const LESSON_ID = "l26"; // Exercises: Input Types
...
<form>
  ...
  <p>Gjinia:</p>
  <label><input type="radio" name="gjinia"> F</label>
  <label><input type="radio" name="gjinia"> M</label>

  <label><input type="checkbox" name="kushtet"> Pranoj kushtet</label>
...
function hasRadioGroup(radios) {
  if (radios.length < 2) return false;
  const names = radios.map((el) => el.getAttribute("name")).filter(Boolean);
  if (names.length < 2) return false;

  return names.some((name) => names.filter((n) => n === name).length >= 2);
}
...
function validate(userHtml) {
  ...
  const radioInputs = Array.from(doc.querySelectorAll('input[type="radio"]'));
  const checkboxInput = doc.querySelector('input[type="checkbox"]');
  const submitInput = doc.querySelector('input[type="submit"], button[type="submit"]');

  const results = {
    form: !!form,
    text: !!textInput,
    password: !!passwordInput,
    radio: hasRadioGroup(radioInputs),
...
function initMonaco() {
  ...
  require(["vs/editor/editor.main"], () => {
    ...
    updateChecklist({ form: false, text: false, password: false, radio: false, checkbox: false, submit: false });
    setStatus("Ready");
  });
}
