...
const LESSON_ID = "l18"; // HTML Lists exercise
...
<h2>Lista ime</h2>
<p>Shembull i thjeshte me lista.</p>
...
function validate(userHtml) {
  ...
  const ulCount = doc.getElementsByTagName("ul").length;
  const olCount = doc.getElementsByTagName("ol").length;
  const ulItems = doc.querySelectorAll("ul li").length;
  const olItems = doc.querySelectorAll("ol li").length;
...
function initMonaco() {
  ...
  require(["vs/editor/editor.main"], () => {
    ...
    updateChecklist({ ul: false, ol: false, ulItems: false, olItems: false });
    setStatus("Ready");
  });
}
