...
const LESSON_ID = "l13"; // HTML Images exercise
...
function hasWidth(img) {
  const widthAttr = (img.getAttribute("width") || "").trim();
  const styleAttr = (img.getAttribute("style") || "").toLowerCase();
  const hasStyleWidth = /width\s*:\s*\d/.test(styleAttr);
  return widthAttr !== "" || hasStyleWidth;
}
...
function validate(userHtml) {
  ...
  const hasImg = images.length >= 1;
  const hasSrc = images.some((img) => (img.getAttribute("src") || "").trim().length > 0);
  const hasAlt = images.some((img) => (img.getAttribute("alt") || "").trim().length > 0);
  const hasWidthValue = images.some((img) => hasWidth(img));
...
function initMonaco() {
  ...
  require(["vs/editor/editor.main"], () => {
    ...
    updateChecklist({ img: false, src: false, alt: false, width: false });
    setStatus("Ready");
  });
}
